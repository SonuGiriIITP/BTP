...
if __name__ == "__main__":
    res = DEM_creator(H=[0.7,0.8,0.9], H_wt=[0.3,0.4,0.2], seed=[5,7,31], elev_range=[0,800],sigma=1,gradient=0, max_level=7, DEMcreator_option='fm2D')
    # header info
    DEM_arr = res[0]
    resolution = 30
    fo = open("DEM.asc","w")
    south = 0 #some arbitrary value
    north = south + resolution*DEM_arr.shape[0]
    west =  0  #some arbitrary value
    east =  west + resolution*DEM_arr.shape[1]
    ...
    g.run_command('r.out.png', input='walk_map@user1', output = fullpath + '/'+'cost_walk')
    g.run_command('r.drain',overwrite=True,input = 'cost_map@user1',output='cost_map_drain',coordinate = '2400,2400')
    g.run_command('r.drain',overwrite=True,input = 'walk_map@user1',output='walk_map_drain',coordinate = '2400,2400')
    g.run_command('r.out.png', input='cost_map_drain@user1', output = fullpath + '/'+'cost_drain')
    g.run_command('r.out.png', input='walk_map_drain@user1', output = fullpath + '/'+'walk_drain')
    g.run_command('r.param.scale',overwrite=True,input='test_DEM@user1',output='morphology',param='feature',size=11)
    g.run_command('r.out.png', input='morphology@user1', output = fullpath + '/'+'morphology')
    g.run_command('r.to.rast3',overwrite=True,input='test_DEM@user1',output='3D')
    g.run_command('r3.out.vtk',input='3D@user1', output = fullpath + '/'+'3D_vtk')
    g.run_command('r3.to.rast',overwrite=True,input='3D@user1',output='2D')
    g.run_command('r.out.png', input='2D_00001@user1', output = fullpath + '/'+'2Dslice')
    g.run_command('r.out.png', input='walk_map_drain@user1', output = fullpath + '/'+'road')
