...
def main():
    ...
    filename = "%s/Original_DEM" % (output_dir)
    pylab.imsave(filename, DEM_Result[0])
    for i in range(0,len(DEM_Result[1])):
        filename = "%s/%sH_%fHwt_%f" % (output_dir,DEM_Result[2][i],DEM_Result[3][i][0],DEM_Result[3][i][1])
        filename = filename.replace('.','')
        pylab.imsave(filename, DEM_Result[1][i])
    ...
    if not Erosion_permission:
        ...
        g.run_command('r.out.png', input='test_DEM@user1', output = fullpath + '/'+output_dir +'/'+'DEM')
        ...
        g.run_command('r.out.png', input='flooded_DEM@user1', output = fullpath + '/'+output_dir +'/'+'flooded_DEM')
        g.run_command('r.out.png', input='DEM_flow_direction@user1', output = fullpath + '/'+output_dir +'/'+'flow_direction')
        g.run_command('r.out.png', input='DEM_sink_watershed@user1', output = fullpath + '/'+output_dir +'/'+'watershed')
        g.run_command('r.out.png', input='DEM_flow_accum@user1', output = fullpath +'/'+ output_dir +'/'+'flow_accumulation')
        g.run_command('r.out.png', input='DEM_tci@user1', output = fullpath + '/'+output_dir +'/'+'tci')
        ...
        f.close()
        print "Flow_accum ",Flow_accum_arr.shape
    ...
    print "DEM_Aspect_arr", DEM_Aspect_arr.shape
    f.close()
    ...
    f.close()
    #image_arr = numpy.zeros((DEM_arr.shape[0],DEM_arr.shape[1],3),dtype='int')
    #image_arr[:,:,0] = DEM_arr[:,:]
    #image_arr[:,:,1] = Display_fields[:,:]
    #scipy.misc.imsave('outfile.jpg', image_arr)
